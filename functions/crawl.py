...
def launch_spider(url, domain, path):
	...
	cmd(f"gospider -a -r --sitemap -q -d 3 -s {url} -o {path}{FOLDER_CRAWL}")
	end_print("GOSPIDER")
...
def launch_gauplus(domain, path):
	...
	cmd(f"gauplus --random-agent -b css,jpg,png,gif,svg,ico,woff,woff2,pdf,ttf,otf,jpeg,JPG,mp4,mp3,avi,eot -o {path}{FOLDER_CRAWL}gauplus {domain}")
	end_print("GAUPLUS")
...
def parse_data_merge_gospider_gauplus(domain, path):
	...
	cmd(f"cat {path}{FOLDER_CRAWL}gospider_removed_js.out | httpx -sc -cl -nc -fr -silent | tee {path}{FOLDER_CRAWL}httpx.http")
